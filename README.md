...
Visit [http://localhost:8000/](http://localhost:8000/). Upload one PDF by button or drag-and-drop. Review the editable fields; export JSON, flattened CSV, or TXT, or copy JSON. Add/remove experience, education, skills, language, certification and project entries. Work and education durations update when their dates are edited. Dark/light theme is the only value saved to browser storage. Documents, API keys and edits remain in tab memory; export before closing. JSON download initiation clears the unsaved-edit banner, but the browser/user controls whether the file is ultimately saved.
...
The dashboard response includes `personalInformation`, `contactInformation`, `experience`, `education`, `skills`, `languages`, `certifications`, `projects`, `summary` and `metadata`. Professional information, confidence scores/percentages and dynamic tables are excluded from the dashboard and its JSON/CSV/TXT exports. Metadata contains page/word/contact counts, extraction method, language, processing time, LLM mode and warnings. Missing string values are empty; unsupported demographic details are not inferred. The separate legacy `/api/v1/resume/parse` evidence schema remains available.

The dashboard does not run dynamic table extraction. Each work and education entry includes a calculated `duration`. Dates in `YYYY-MM` use elapsed calendar-month differences; dates in `YYYY-MM-DD` count completed day-aligned months (or elapsed days for periods under one month). `Present` uses the current UTC date. Missing, invalid, reversed or year-only dates leave duration empty. Date edits recalculate duration immediately and exports include the updated value; durations are read-only derived fields.
...
Name extraction without a configured spaCy model is heuristic. First/last names are split from extracted names, nationality requires an explicit label, and durations are calculated from dates. Unsupported facts such as employment type are not guessed. Certificates and projects are preserved as section lines rather than fully resolving their multi-line relationships. Legacy API evidence scores are not probabilities of correctness.
